...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private boolean processAuthentication(String jwt, HttpServletRequest request, HttpServletResponse response) throws IOException {
        try {
            //JWT 토큰을 파싱하여 Claims(노큰에 담긴 정보) 추출
    ...
    private void setAuthentication(Claims claims){
        ...
        Authentication authenticationToken = new JwtAuthenticationToken(authUser, null);
    ...
    private void sendErrorResponse(HttpServletResponse response, HttpStatus status, String message) throws IOException {
        ...
        errorResposne.put("status", status.name());
        errorResposne.put("code", status.name());
        errorResposne.put("message", status.name());
        response.getWriter().write(objectMapper.writeValueAsString(errorResposne));
    }
}
